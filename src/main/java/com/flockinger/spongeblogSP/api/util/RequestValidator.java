...
public class RequestValidator {
  ...
  public void validateRequestBody(BindingResult bindingResult) throws DtoValidationFailedException {
    if (bindingResult.hasErrors()) {
      throw new DtoValidationFailedException("Invalid field entries!",
  ...
  public <T extends Enum<T>> void assertStatus(Class<T> statusClass, String status)
    ...
    if (!Enums.getIfPresent(statusClass, status).isPresent()) {
      throw new DtoValidationFailedException(
          "Invalid " + statusClass.getSimpleName() + ": " + status, new ArrayList<FieldError>());
    }
  }
