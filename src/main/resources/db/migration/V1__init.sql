...
create table user (id bigint not null auto_increment, login VARCHAR(150) not null check (login<=150), nick_name varchar(255), password varchar(255) not null, registered datetime not null, primary key (id));
...
alter table post add constraint UK7cw8cbopgbty5qo4hkqxwp8ed unique (id, title);
create index IDX1wdpsed5kna2y38hnbgrnhi5b on tag (name);
alter table tag add constraint UK9c155tcbv3axt5uymdd9t3rqu unique (id, name);
create index IDXew1hvam8uwaknuaellwhqchhb on user (login);
alter table user add constraint UK6e59of1jvk758h7vp5h0phi59 unique (id, login);
