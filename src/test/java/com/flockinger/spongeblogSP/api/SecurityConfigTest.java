...
public class SecurityConfigTest extends BaseControllerTest {
	...
	/*
	@Test
	public void testGetImages_withAnonymous_shouldReturnCorrect() throws Exception {
	...
	@Test
	public void testGetImagesWithKey_withAnonymous_shouldReturnCorrect() throws Exception {
		mockMvc.perform(get("/api/v1/images/1_bla.jpg")
	               .contentType(jsonContentType))
	                .andExpect(status().isOk());
	}
	
	@Test
	public void testGetImageLink_withAnonymous_shouldReturnCorrect() throws Exception {
		mockMvc.perform(get("/api/v1/images/link/1_bla.jpg")
	               .contentType(jsonContentType))
	                .andExpect(status().isOk());
	}*/
	...
	public void testChangeCategory_withAdmin_shouldReturnWork() throws Exception {		
		...
		mockMvc.perform(put("/api/v1/categorie/rewind/1")
	               .contentType(jsonContentType).headers(getAdminHeaders()))
...
/*	
	@Test
	public void testChangeImage_withAnonymous_shouldReturn401() throws Exception {
	...
	@Test
	public void testChangeImage_withUser_shouldReturn403() throws Exception {
		mockMvc.perform(post("/api/v1/images")
	               .contentType(jsonContentType).headers(getUserHeaders()))
	                .andExpect(status().isForbidden());
	...
	@Test
	@FlywayTest(locationsForMigrate = { "/db/testfill/" })
	public void testChangeImage_withAdmin_shouldReturnWork() throws Exception {		
		mockMvc.perform(post("/api/v1/images")
	               .contentType(jsonContentType).headers(getAdminHeaders()))
	                .andExpect(status().isBadRequest());
	...
	public void testChangeUser_withAdmin_shouldReturnWork() throws Exception {	
		...
		mockMvc.perform(put("/api/v1/users/rewind/2")
	               .contentType(jsonContentType).headers(getAdminHeaders()))
	                .andExpect(status().isConflict());
	}
	...
	private HttpHeaders getAdminHeaders(){
		...
		headers.add(HttpHeaders.AUTHORIZATION, "Basic " + new String(Base64.encode(("flo:secret").getBytes())));
		return headers;
	}
	
	private HttpHeaders getUserHeaders(){
		HttpHeaders headers = new HttpHeaders();
		headers.add(HttpHeaders.AUTHORIZATION, "Basic " + new String(Base64.encode(("nobody:secret").getBytes())));
		return headers;
	}
}
